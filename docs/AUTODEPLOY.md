...
workflow `CI` для push в ветку `main` и только когда repository variable
`AUTODEPLOY_ENABLED` имеет значение `true`.

На сервер передаётся `scripts/deploy.sh` именно из протестированного commit.
Скрипт:
...
После добавления всех SSH secrets откройте:
...
Пока этой переменной нет или её значение отличается от `true`, workflow
`Deploy production` корректно пропускается и не создаёт ошибочный деплой.

## 5. Проверьте серверный репозиторий
...
После добавления secrets и `AUTODEPLOY_ENABLED=true` можно запустить деплой
вручную:
