...
POSITIVE_RULES: tuple[Rule, ...] = (
    ...
    Rule(
        "Выражена потребность в цифровом продукте",
        30,
        (
            r"\b(?:хочу|хотим|хотел(?:а|и)?\s+бы)\s+(?:сделать\s+|разработать\s+|заказать\s+)?(?:бота?|сервис|систему|приложение|скрипт|парсер|crm|mini\s*app)\b",
            r"\b(?:нужен|нужна|нужно|нужны|требуется)\s+(?:бот|бота|сервис|система|приложение|скрипт|парсер|crm|mini\s*app|решение)\b",
            r"\b(?:сделать|разработать|написать|доработать|заказать)\s+(?:бота?|сервис|систему|приложение|скрипт|парсер|crm|mini\s*app)\b",
        ),
    ),
    Rule(
    ...
    Rule(
        ...
        (
            ...
            r"\b(?:бот|бота|боту|ботом|боты|ботов)\b",
            ...
            r"\bпарсер\b|\bпарсинг\b|\bbackend\b|\bбэкенд\b",
        ),
    ),
    Rule(
        ...
        (
            ...
            r"\b(?:анализир|сравнив|монитор|отслежив|собир|парс|гугл)\w*\b",
        ),
    ),
    Rule(
